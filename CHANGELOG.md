...
- **Portable build script (Req 31)**: `build.rs` uses `chrono::Utc::now().format(...)` instead of shelling out to `/bin/date`. Build now works on any Unix or Windows target without a system `date` binary. Output format preserved.
