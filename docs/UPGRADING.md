...
|---|---|
| 2.8.2 | The first GET of an object cached by a PUT or multipart upload now makes one extra round trip to S3 (a conditional request) before serving, where previously it was served straight from cache with no S3 contact. Subsequent reads of the same object are unaffected. |
| 2.8.0 | Expired sequential and byte-range GETs now revalidate with a conditional S3 request and serve cached bytes on `304`, so body transfers fall while conditional requests and `ttl_revalidations_total` rise; a lowered or zeroed `get_ttl` now also takes effect on ranged reads of already-cached objects; orphan range files are kept at startup rather than swept, so watch free space on a cache volume that accumulates them; `cache.ram_cache_verification_interval` is deprecated and logs a startup warning; and four startup and consolidation log lines change fields. |
