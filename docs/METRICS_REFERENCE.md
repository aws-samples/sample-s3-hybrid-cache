...
| `ttl_revalidations_total` | counter | Conditional GET/HEAD sent to S3 before serving cached bytes. Two causes, not one: TTL expiry, and a write-cached entry whose `Last-Modified` is not yet known. The second revalidates while the entry is still **fresh** by TTL, so this counter moves on the first read of every written object even under a generous `get_ttl` — see [CACHE_FRESHNESS.md](CACHE_FRESHNESS.md) |
